...
function create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append)
    ...
    if _type == 'Joker' then
        ...
        if (area == G.shop_jokers) or (area == G.pack_cards) then 
            local eternal_perishable_poll = pseudorandom((area == G.pack_card and 'packetper' or 'etperpoll')..G.GAME.round_resets.ante)
            ...
            if G.GAME.modifiers.enable_rentals_in_shop and pseudorandom((area == G.pack_card and 'packssjr' or 'ssjr')..G.GAME.round_resets.ante) > 0.7 then
