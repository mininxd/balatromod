local talisman_path = "mods/Talisman"

-- "Borrowed" from Trance
function load_file_with_fallback2(a, aa)
    local success, result = pcall(function() return assert(load(love.filesystem.read(a)))() end)
    if success then
        return result
    end
    local fallback_success, fallback_result = pcall(function() return assert(load(love.filesystem.read(aa)))() end)
...
local config_read_result = love.filesystem.read(talisman_path.."/config.lua")
...
function tal_get_blind_amount(ante)
  local k = to_big(0.75)
  ...
  function get_blind_amount(ante)
    if G.GAME.modifiers.scaling and (G.GAME.modifiers.scaling ~= 1 and G.GAME.modifiers.scaling ~= 2 and G.GAME.modifiers.scaling ~= 3) then return tal_get_blind_amount(ante) end
  ...
  function scale_number(number, scale, max, e_switch_point)
    ...
    if not e_switch_point and number.arraySize and number:arraySize() > 2 then --this is noticable faster than >= on the raw number for some reason
    ...
    elseif math.abs(to_big(number)) >= to_big(e_switch_point or G.E_SWITCH_POINT) then
      if number.arraySize and number:arraySize() <= 2 and (number.array[1] or 0) <= 999 then --gross hack
        scale = scale*math.floor(math.log(max*10, 10))/7 --this divisor is a constant so im precalcualting it
      else
...
function tal_calculate_individual_effect(effect, scored_card, key, amount, from_edition)
